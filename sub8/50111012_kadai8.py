...
def make_top_page(indust_code, indust_class):
    ...
    title, date, desc, link, link_t,\
          cpyrgt, filename, top_directory, prefix = get_config(CONFIGFILE)
    ...
    html_text.append("<h2>"+link_t+"("+date+")</h2>")
    html_text.append("<ul>")
    for class_name, class_code in zip(class_dict, code_dict):
        html_text.append("<li><a href=\""+str(prefix)+str(class_code)+".html\">")
...
def make_category_page(indust_class, indust_code, store_name, tel, register_date, url):
    """
    カテゴリページを作る関数
    ...
    for i in range(number_of_category):
        ...
        html_row.append("\n".join(row_text))
        for j in range(len(name_categorized)):
            li_text = "<li>"
            ...
            li_text += " TEL: " + str(tel_categorized[i][j]) + "</ul>\n"
        ...
        filename = top_directory + "/" + prefix + str(code_list) + ".html"
        output_html(html_code, filename)
...
def main():
    ...
    make_category_page(industclass, industcode, name, tel, register_date, weburl)
